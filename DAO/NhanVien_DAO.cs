...
namespace DAO
{
    public class NhanVien_DAO
    {
        ...
        public static NhanVien_DTO TimNV(string ID)
        {
            string QueryString = string.Format($"select * from NhanVien where  [MaNV] ='{ID}'");
            conn = DataProvider.OpenConnection();
            DataTable dt = DataProvider.LayDataTable(QueryString, conn);
            if (dt.Rows.Count == 0)
                return null;
            ...
            nhanVien.MaNhanVien = dt.Rows[0]["MaNV"].ToString();
            nhanVien.TenNhanVien = dt.Rows[0]["Họ và tên"].ToString();
            nhanVien.NgaySinh = dt.Rows[0]["Ngày sinh"].ToString();
            nhanVien.GioiTinh = dt.Rows[0]["Giới tính"].ToString();
            nhanVien.SoDienThoai = dt.Rows[0]["Số điện thoại"].ToString();
            nhanVien.ChucVu = dt.Rows[0]["Chức vụ"].ToString();
            nhanVien.Luong = dt.Rows[0]["Lương"].ToString();
            nhanVien.DiaChi = dt.Rows[0]["Địa chỉ"].ToString();
            DataProvider.CloseConnection(conn);
            return nhanVien;
        }
    }
